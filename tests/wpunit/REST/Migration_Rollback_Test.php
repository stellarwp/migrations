<?php
/**
 * REST API tests for the Migration rollback endpoint.
 *
 * @since 0.0.1
 *
 ...
 * @package StellarWP\Migrations\Tests\REST
 */
class Migration_Rollback_Test extends REST_Test_Case {
	...
	public function it_should_return_401_for_guest_user(): void {
		...
		$response = $this->do_rest_api_request( '/migrations/tests_simple_migration/rollback', 'POST' );
	 ...
	 * @test
	 */
	public function it_should_rollback_a_simple_migration(): void {
		$this->set_admin_user();

		$data = $this->assert_endpoint( '/migrations/tests_simple_migration/rollback', 'POST', 200 );
		...
		$this->assertEquals( 'down', $data['operation'] );
		$this->assertArrayHasKey( 'from_batch', $data );
		$this->assertEquals( 1, $data['from_batch'] );
		$this->assertArrayHasKey( 'to_batch', $data );
		$this->assertEquals( 1, $data['to_batch'] );
		$this->assertArrayHasKey( 'batch_size', $data );
		$this->assertEquals( 1, $data['batch_size'] );
	}

	/**
	 * @test
	 */
	public function it_should_create_execution_record_when_rolling_back_migration(): void {
		$this->set_admin_user();

		$this->assert_endpoint( '/migrations/tests_simple_migration/rollback', 'POST', 200 );
	...
	public function it_should_return_error_for_nonexistent_migration(): void {
		...
		$data = $this->assert_endpoint( '/migrations/nonexistent_migration_xyz/rollback', 'POST', 400 );
	 ...
	 * @test
	 */
	public function it_should_rollback_migration_with_from_batch_parameter(): void {
		$this->set_admin_user();

		$data = $this->assert_endpoint(
			'/migrations/tests_multi_batch_migration/rollback',
			'POST',
			200,
			[ 'from-batch' => 2 ]
		);
	 ...
	 * @test
	 */
	public function it_should_rollback_migration_with_to_batch_parameter(): void {
		$this->set_admin_user();
	 ...
	 * @test
	 */
	public function it_should_rollback_migration_with_batch_size_parameter(): void {
		$this->set_admin_user();
	 ...
	 * @test
	 */
	public function it_should_rollback_migration_with_all_batch_parameters(): void {
		$this->set_admin_user();
